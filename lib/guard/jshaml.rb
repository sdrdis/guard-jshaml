...
module Guard
  class JSHaml < Guard
    autoload :Notifier, 'guard/jshaml/notifier'
    ...
    def run_on_changes(paths)
      paths.each do |file|
        ...
        message = "Successfully compiled jshaml to js!\n"
    ...
    def compile_haml file
      begin
        ...
        engine  = haml_to_js.new(@options[:jshaml_options] || {})#::Haml::Engine.new(content, (@options[:haml_options] || {}))
        engine.convert_from_string(content)
      rescue StandardError => error
        message = "JSHAML compilation failed!\nError: #{error.message}"
